...
from ..constants import (AccountTxFlags, DerivationType, KeyInstanceFlag,
    TransactionInputFlag, TransactionOutputFlag, PaymentFlag, ScriptType, TxFlags, WalletEventFlag,
    WalletEventType)
from ..logs import logs
from .sqlite_support import SQLITE_EXPR_TREE_DEPTH, SQLITE_MAX_VARS, DatabaseContext, \
    CompletionCallbackType
from .util import collect_results, flag_clause, read_rows_by_id, update_rows_by_id
...
class KeyInstanceTable(BaseDatabaseAPI):
    ...
    CREATE_SQL = ("INSERT INTO KeyInstances "
        ...
        "flags, description, date_created, date_updated) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)")
...
class AsynchronousFunctions(BaseDatabaseAPI):
    def _insert_transaction(self, db: sqlite3.Connection, tx_row: AddTransactionRow,
        ...
        assert tx_row.tx_data is not None
        ...
        db.execute("INSERT INTO Transactions (tx_hash, version, locktime, tx_data, flags, "
            "block_height, block_position, description, fee_value, date_created, "
    ...
    def _import_transaction(self, db: sqlite3.Connection, tx_row: AddTransactionRow,
        ...
        """
        await self._insert_transaction(db, tx_row, txi_rows, txo_rows)
        await self._link_transaction(db, tx_row, txi_rows, txo_rows)
